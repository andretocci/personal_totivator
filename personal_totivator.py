...
class personal_totivator:
  ...
  def plot_describe(self, plot_type = sns.boxplot):
    ...
    plot_type(y="Atividades", x="tempo", data=df, palette= self.color_palette, linewidth=2, ax = axs[2])
    ...
    df3 = df.groupby(['Atividades'])['tempo'].mean().reset_index() #.agg({'tempo': ['sum']})
    sns.barplot(y="Atividades", x="tempo", data= df3, palette= self.color_palette, ax = axs[1])

    titles = ['Tempo Investido', 'Tempo Médio', 'Distribuição']
    for axe, title in zip(axs, titles):
      # axe.legend(frameon=True, fancybox=True,loc='lower right')
      # axe.set_xlabel('')
      # axe.set_ylabel('')
      axe.set_title(title)
